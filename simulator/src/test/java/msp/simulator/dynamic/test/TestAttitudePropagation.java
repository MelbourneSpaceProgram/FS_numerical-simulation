...
public class TestAttitudePropagation {
	...
	public void testSimpleAcceleration() throws Exception {
		...
		Dashboard.setIntegrationTimeStep(1./20.);
		...
		torqueScenario.add(new Step(0., accDuration + 0.1, rotVector));
		...
		Vector3D expectedSpin = new Vector3D(expectedRotAcc).scalarMultiply(accDuration);
		System.out.println(expectedSpin.toString());
		System.out.println(Arrays.toString(finalState.getAdditionalState("Spin")));
		System.out.println(finalState.getAdditionalState("Spin")[0]);
		
		
		/* Checking Spin */
		Assert.assertArrayEquals(
				expectedSpin.toArray(), 
